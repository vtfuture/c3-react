...
var C3Chart = React.createClass({
	...
	drawGraph: function drawGraph() {
		switch (this.props.type) {
		    ...
		    case "area-line":
		    case "area-spline":
				this._chart = this.drawGraphLine();
				break;
			case "bar":
			    this._chart = this.drawGraphBar();
				break;
			case "pie":
			    this._chart = this.drawGraphPie();
				break;
			case "multiBar":
			    this._chart = this.drawGraphMultiBar();
				break;
			case "lineBar":
			    this._chart = this.drawGraphlLineBar();
				break;
		}
	},
	updateGraph: function updateGraph(nextProps) {
	    switch (this.props.type) {
	        case "line":
	        case "spline":
	        case "area-line":
	        case "area-spline":
	            this.updateGraphLine(nextProps);
	            break;
	        case "bar":
	            this._chart = this.drawGraphBar();
	            break;
	        case "pie":
	            this.updateGraphPie(nextProps);
	            break;
	        case "multiBar":
	            this._chart = this.drawGraphMultiBar();
	            break;
	...
	updateGraphLine: function updateGraphLine(nextProps) {
        console.log("update line");

        var graphObject = this.graphObject();

        var graphObjectData = _.merge(nextProps.data, { type: this.props.type });
        ...
        this._chart.load(graphObject.data)
    },

	updateGraphPie: function updateGraphPie(nextProps) {
        console.log("update pie");

        var graphObject = this.graphObject();
        var graphObjectData = {
            columns: this.pieChartDataPreparator(nextProps.data[0].values),
            type: "pie"
        };
	...
	componentWillReceiveProps(nextProps) {
	    if (nextProps.expanded == this.props.expanded && this._chart != null) {
